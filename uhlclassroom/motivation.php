...
?>
    ...
    <td width="80%" valign="top">
        ...
        <div class="allVids">
                <?php
                require_once('connect.php');
                $getTopics=mysqli_query($connection,"select distinct maintopic from vid_lecture where Subject='Words of Wisdom'");
                if($getTopics->num_rows>0){
                    while($topic=mysqli_fetch_array($getTopics)){
                        $sql = "SELECT * FROM `vid_lecture` where maintopic='".$topic['maintopic']."'";
                        $result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
                        ?>
                        <div class="videoTopicWise row">
                            <h4 class="videoTopicTitle"><?php echo $topic['maintopic']?></h4>
                            <?php
                            while($row = $result->fetch_assoc()){
                                ?>
                                <div class="col-md-3">
