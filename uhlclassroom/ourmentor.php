...
<?php
include('connect.php');
$sql = "SELECT user_id,(select username from login where login.id=mentorform.user_id) as username,fullname,(select image from mentor_image where mentorform.user_id=mentor_image.user_id) as image from mentorform";
    $result = mysqli_query($connection, $sql)or die(mysqli_error($connection));
  if ($result->num_rows > 0) {?>
 ...
 <tr valign='middle' align='center'>
               <th class='text-center col-md-4 danger' >MENTOR NAME</th>
...
<?php if ($_SESSION["usertype"]=='Admin') {?> <th class='text-center col-md-4 danger'>DELETE USER</th><?php }
  ...
  <?php  while ($data = $result->fetch_assoc()){?>
        ...
        ?>
        <td class="text-center col-md-4 warning" style="vertical-align:middle;">
            <p><a style="color: #ffffff" class="btn btn-lg btn-info btn-block" href='mentorhome.php?id=<?php echo $data['user_id'];?>'>
        <?php
            echo $data["username"];
        ?>
        </a> </br></p>
