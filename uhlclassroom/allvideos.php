...
if (!$_SESSION['user_id']){
    ...
    header("location:login.php?msg= You are not yet verified by admin.");
}
?>
    ...
    <td width="80%" valign="top">
        <div class="container">
            <div class="allVids">
                <?php
                require_once('connect.php');
                $getSubjects=mysqli_query($connection,"select distinct Subject from vid_lecture");
                if($getSubjects->num_rows>0){
                    while($subjectArr=mysqli_fetch_array($getSubjects)){
                        $subject=$subjectArr['Subject'];
                        $sql = "SELECT * FROM `vid_lecture` where Subject='".$subject."'";
                        $result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
                        if($subject=="11maths"){
                            $subject="Mathematics 11th Class";
                        }
                        else if($subject=="12maths"){
                            $subject="Mathematics 12th Class";
                        }
                        else if($subject=="11phys"){
                            $subject="Physics 11th Class";
                        }
                        else if($subject=="12phys"){
                            $subject="Physics 12th Class";
                        }
                        else if($subject=="11chem"){
                            $subject="Chemistry 11th Class";
                        }
                        else if($subject=="12chem"){
                            $subject="Chemistry 12th Class";
                        }
                        ?>
                        <div class="videoTopicWise row">
                            <h4 class="videoTopicTitle"><?php echo $subject?></h4>
                            <?php
                            while($row = $result->fetch_assoc()){
                                ?>
                                <div class="col-md-3">
                                    <a href="video_single.php?vid_id=<?php echo $row['vid_id'];?>">
                                        ...
                                        <?php
                                        echo '<span style=" font-size: 18px;"><b>'.$row["Name"].'</b><br>';
                                        ?></a> <br><br> </div>
                                        <?php
                                    }
                                    ?>
