...
<head>
    ...
    <title>Our Vigyan Jyoti Students</title>
...
<table width="100%">
    ...
    <td width="80%" valign="top">
        ...
        <table>
            ...
            <?php
            include('connect.php');
            $sql = "SELECT  fullname, stuform.user_id,login.username,login.usertype,(select image from stu_image where stu_image.user_id=stuform.user_id) as image from stuform INNER JOIN login ON stuform.user_id = login.id where login.usertype='Vigyan Jyoti Student'";
            $result = mysqli_query($connection, $sql)or die(mysqli_error($connection));
            if ($result->num_rows > 0) {?>
               <table border='3' cellspacing='10' cellpadding='10' width='500' bgcolor="#FFEBCD" class="table table-striped table-responsive" valign='middle' >
                   <tr valign='middle' align='center'>
                     <th class='text-center col-md-4 danger' >STUDENT NAME</th>
                     <th class='text-center col-md-4 danger'>IMAGE</th>
                     <th class='text-center col-md-4 danger'>USERNAME</th>
                     <?php if ($_SESSION["usertype"]=='Admin') {?> <th class='text-center col-md-4 danger'>DELETE USER</th><?php }
                     ?>
                 </tr>
                 <?php  while ($data = $result->fetch_assoc()){?>
                    <tr>
                       <td class="align-middle text-center col-md-4 warning vcenter" style="vertical-align:middle;"><?php 
                       echo $data["fullname"];
                       if($data['image']){?>
                           <td class="warning" align='middle' > <img src="<?php echo $data['image'];?>" width="150" height="200" >
                            <?php
                        }
                        else{?>
                           <td class="warning" align='middle' > <img src="images/slider/pp.png" width="150" height="150" >
                            <?php
                        }
                        ?>
                        
                        
                        <td class="text-center col-md-4 warning" style="vertical-align:middle;"><p><a style="color: #ffffff" class="btn btn-lg btn-info btn-block" href='stuhome.php?id=<?php echo $data['user_id'];?>'>
                            <?php
                            echo $data["username"];
                            ?>
                        </a> </br></p><p>
                            ...
                            if(isset($_SESSION["user_id"])){
                                ?>
                                <a style="color: #ffffff" class="btn btn-lg btn-info btn-block" href='new_pm.php?receiver=<?php echo $data['username'];?>'>Chat</a> </p></td>
                                <?php
                            }
                            ?>
                            <?php if ($_SESSION["usertype"]=='Admin') {?>
                                <td class="text-center col-md-4 warning" style="vertical-align:middle;"><a style="color: #ffffff;" class="btn btn-lg btn-info btn-block" href='deluser2.php?id=<?php echo $data['user_id'];?>'>Delete mentor </a></td>
                            <?php }
                            ?>
                        </td>
                        <?php
